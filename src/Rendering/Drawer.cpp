...
namespace Drawer {
    void draw_text(float a_x, float a_y, const char* a_text, ImU32 a_color, float a_font_size,
                           DrawArgs a_drawArgs, bool a_center_text) {
        ...
        ImU32 shadowCol = Styling::Wheel::TextShadowColor;
...
namespace Drawer {
    void draw_text_block(float a_x, float a_y, std::string& a_text, ImU32 a_color, float a_font_size,
                                 float a_line_spacing, float a_line_length, DrawArgs a_drawArgs) {
        if (a_text.empty()) {
            return;
        }
        if (a_drawArgs.alphaMult == 0.f || a_font_size == 0) {
        ...
        for (const auto& line : text_lines) {
            draw_text(a_x, line_y, line.c_str(), a_color, a_font_size, a_drawArgs, true);
            line_y += (font_height + a_line_spacing);
        }
    }

    void draw_texture(ID3D11ShaderResourceView* a_texture, ImVec2 a_center, float a_offset_x, float a_offset_y,
                              ImVec2 a_size, ImU32 a_color, DrawArgs a_drawArgs) {
        ...
        Utils::Color::MultAlpha(a_color, a_drawArgs.alphaMult);
        ImGui::GetWindowDrawList()->AddImageQuad(a_texture, pos[0], pos[1], pos[2], pos[3], uvs[0], uvs[1], uvs[2],
                                                 uvs[3], a_color);
    }

    void draw_arc(ImVec2 center, float radius_min, float radius_max, float inner_ang_min, float inner_ang_max,
                          float outer_ang_min, float outer_ang_max, ImU32 a_color, uint32_t segments,
                          DrawArgs a_drawArgs) {
        auto drawList = ImGui::GetWindowDrawList();
        ...
        for (int iSeg = 0; iSeg <= segments; ++iSeg) {
            ...
            drawList->PrimWriteVtx(ImVec2(center.x + fCosInner * radius_min, center.y + fSinInner * radius_min),
                                   vTexUvWhitePixel, a_color);
            drawList->PrimWriteVtx(ImVec2(center.x + fCosOuter * radius_max, center.y + fSinOuter * radius_max),
                                   vTexUvWhitePixel, a_color);
        }
    }
}  // namespace Drawer
...
namespace Drawer {
    void draw_arc_gradient(ImVec2 center, float radius_min, float radius_max, float inner_ang_min,
                                   float inner_ang_max, float outer_ang_min, float outer_ang_max, ImU32 color_start,
                                   ImU32 color_end, uint32_t segments, DrawArgs a_drawArgs) {
        const float fAngleStepInner = (inner_ang_max - inner_ang_min) / segments;
        const float fAngleStepOuter = (outer_ang_max - outer_ang_min) / segments;
        ...
        for (int iSeg = 0; iSeg <= segments; ++iSeg) {
            ...
            drawList->PrimWriteVtx(ImVec2(center.x + fCosOuter * radius_max, center.y + fSinOuter * radius_max),
                                   vTexUvWhitePixel, colorOuter);
        }
    }

    void draw_circle_filled(const ImVec2& center, float radius, ImU32 color, int segments,
                                    DrawArgs a_drawArgs) {
        Utils::Color::MultAlpha(color, a_drawArgs.alphaMult);
        ImGui::GetWindowDrawList()->AddCircleFilled(center, radius, color, segments);
    }

    void draw_triangle_filled(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, ImU32 color,
                                      DrawArgs a_drawArgs) {
        Utils::Color::MultAlpha(color, a_drawArgs.alphaMult);
